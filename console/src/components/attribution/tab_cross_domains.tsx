      ...
      .then(([sqlQuery, resultSet]: any[]) => {
        ...
        previousData.forEach((previousRow) => {
          ...
          let existingRow = rows.find((r) => {
            ...
            return r.currentPeriod['Order.domains_funnel'] === previousRow['Order.domains_funnel']
          })
    ...
    <>
      ...
      <Fullscreenable>
        <Table
          ...
          rowKey={(record) => record.currentPeriod['Order.domains_funnel']}
          columns={[
            {
              title: 'Domains paths (top 300)',
              key: 'path',
              render: (row) => {
                const domains = row.currentPeriod['Order.domains_funnel'].split('~')
                return domains.map((domainID: string, i: number) => {
                  const domain = domainsAsMap[domainID]
                  return (
...
export default TabAttributionCrossDomains
