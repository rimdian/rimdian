var Rimdian = (function () {
  ...
  */
  ...
  trackPageview:function trackPageview(a){if(Rimdian.log("info","trackPageview()",a),!Rimdian.isReady)return Rimdian.log("debug","RMD is not yet ready for trackPageview, queuing function..."),void Rimdian._execWhenReady(function(){return Rimdian.trackPageview(a)});// persist existing pageview duration if any, or discard it if not active
  ...
  var c=Math.round((new Date().getTime()-Rimdian.currentPageviewVisibleSince.getTime())/1e3);Rimdian.currentPageviewDuration+=c,Rimdian.currentPageview.duration=Rimdian.currentPageviewDuration,Rimdian.currentPageview.updated_at=b,Rimdian.currentSession.duration||(Rimdian.currentSession.duration=0),Rimdian.currentSession.duration+=c,Rimdian.currentSession.updated_at=b,Rimdian.itemsQueue.add("pageview",__assign({},Rimdian.currentPageview)),Rimdian.currentPageviewVisibleSince=void 0,Rimdian.currentPageviewDuration=0;}// deep clone pageview
  var d=JSON.parse(JSON.stringify(a||{}));d.external_id=Rimdian.uuidv4(),d.created_at=b;var e=Rimdian.getReferrer();e&&(d.referrer=e),d.title||(d.title=document.title),d.page_id||(d.page_id=window.location.href),d.title=d.title.replace(/\\"/,"\""),d.product_price&&0<d.product_price&&(d.product_price=Math.round(100*d.product_price)),Rimdian.isPageVisible()&&(Rimdian.currentPageviewVisibleSince=new Date),Rimdian.currentPageview=d,Rimdian.currentUser.last_interaction_at=b,Rimdian.setUserContext(Rimdian.currentUser),Rimdian.currentSession.pageviews_count||(Rimdian.currentSession.pageviews_count=0),Rimdian.currentSession.interactions_count||(Rimdian.currentSession.interactions_count=0),Rimdian.currentSession.pageviews_count++,Rimdian.currentSession.interactions_count++,Rimdian.currentSession.updated_at=b,Rimdian.setSessionContext(Rimdian.currentSession),Rimdian.itemsQueue.add("pageview",__assign({},d));},// tracks the current customEvent
  trackCustomEvent:function trackCustomEvent(a){if(Rimdian.log("info","trackCustomEvent()",a),!Rimdian.isReady)return Rimdian.log("debug","RMD is not yet ready for trackCustomEvent, queuing function..."),void Rimdian._execWhenReady(function(){return Rimdian.trackCustomEvent(a)});if(a&&!a.label)return void Rimdian.log("error","customEvent label is required");// deep clone customEvent
  var b=JSON.parse(JSON.stringify(a||{})),c=new Date().toISOString();b.external_id=Rimdian.uuidv4(),b.created_at=c,b.label=b.label.replace(/\\"/,"\""),b.string_value&&(b.string_value=b.string_value.replace(/\\"/,"\"")),b.non_interactive||(Rimdian.currentUser.last_interaction_at=c,Rimdian.setUserContext(Rimdian.currentUser),!Rimdian.currentSession.interactions_count&&(Rimdian.currentSession.interactions_count=0),Rimdian.currentSession.interactions_count++,Rimdian.currentSession.updated_at=c,Rimdian.setSessionContext(Rimdian.currentSession)),Rimdian.itemsQueue.add("custom_event",__assign({},b));},trackCart:function trackCart(a){if(Rimdian.log("info","trackCart()",a),!Rimdian.isReady)return Rimdian.log("debug","RMD is not yet ready for trackCart, queuing function..."),void Rimdian._execWhenReady(function(){return Rimdian.trackCart(a)});// check if data exists and is an object
  ...
  setUserContext:function setUserContext(a){if(Rimdian.log("info","setUserContext()",a),!Rimdian.isReady)return Rimdian.log("debug","RMD is not yet ready for setUserContext, queuing function..."),void Rimdian._execWhenReady(function(){return Rimdian.setUserContext(a)});// replace user_centric_consent by consent_all
  ...
  if(Rimdian.log("info","onReady() called"),!Rimdian.isBrowserLegit())return void Rimdian.log("warn","Browser is not legit");// merge cfg with default config & validate
