var Rimdian = (function () {
  ...
  */
  ...
  b.updated_at=new Date().toISOString(),Rimdian.currentDevice=b,Rimdian._setCookie("device",JSON.stringify(Rimdian.currentDevice),OneYearInSeconds);},setSessionContext:function setSessionContext(a){if(!Rimdian.isReady)return Rimdian.log("debug","RMD is not yet ready for setSessionContext, queuing function..."),void Rimdian._execWhenReady(function(){return Rimdian.setSessionContext(a)});// merge currentSession with data
  ...
  b.updated_at=new Date().toISOString(),Rimdian.currentSession=b,Rimdian.log("info","RMD updated session is",b),Rimdian._setCookie("session",JSON.stringify(Rimdian.currentSession),Rimdian.config.session_timeout);},// check if user ID has changed
  ...
  // }
  return Rimdian.log("info","new authenticated user detected, reset context",a),Rimdian.currentDevice=void 0,Rimdian._localStorage.remove("device"),Rimdian._createDevice(),Rimdian.currentSession=void 0,Rimdian._deleteCookie("session"),Rimdian._handleSession(),Object.keys(a).forEach(function(b){"string"==typeof a[b]&&""===a[b]&&delete a[b];}),Rimdian.currentUser=__assign({},a),void(void 0===Rimdian.currentUser.created_at&&(Rimdian.currentUser.created_at=new Date().toISOString()));// create a user_alias if previous user was anonymous
  ...
  //   return
  // }
  Rimdian.currentUser=b,Rimdian._setCookie("user",JSON.stringify(Rimdian.currentUser),OneYearInSeconds);},// enqueue the current user
  ...
  b.user_agent!==navigator.userAgent&&(b.updated_at=new Date().toISOString()),b.user_agent=navigator.userAgent,b.language=navigator.language,b.ad_blocker=Rimdian.hasAdBlocker(),b.resolution=window.screen&&window.screen.width&&window.screen.height?window.screen.height>window.screen.width?window.screen.height+"x"+window.screen.width:window.screen.width+"x"+window.screen.height:void 0;for(// create dataImport batches of 10 items maximum
  var c=[],d=[];0<Rimdian.itemsQueue.items.length;)d.push(Rimdian.itemsQueue.items.shift()),10<=d.length&&(c.push(d),d=[]);// add remaining items to last batch
  ...
  // update user cookie
  a.forEach(function(a){a.value&&""!==a.value&&(Rimdian.currentUser[a.key]=a.value);}),Rimdian._setCookie("user",JSON.stringify(Rimdian.currentUser),OneYearInSeconds);},// retrieve existing device ID or create a new one
  // 1. id found in URL
  ...
  if(a&&""!==a)return Rimdian.log("info","found device ID in URL",a),Rimdian.currentDevice={external_id:a,created_at:new Date().toISOString(),user_agent:navigator.userAgent},void Rimdian._setCookie("device",JSON.stringify(Rimdian.currentDevice),OneYearInSeconds);// 2. ID found in cookie
  var b=Rimdian._getCookie("device");if(b&&""!==b){// check if we already had a legacy client ID in a cookie
  if(Rimdian.log("info","found device ID in cookie",b),Rimdian.currentDevice=JSON.parse(b),!0===Rimdian.config.from_cm){var c=Rimdian._getCookie("_cm_cid");if(c&&""!==c){Rimdian.currentDevice.external_id=c;// extract its creation timestamp
  var d=Rimdian._getCookie("_cm_cidat");d&&""!==d&&(Rimdian.currentDevice.created_at=new Date(parseInt(d,10)).toISOString());}}return void Rimdian._setCookie("device",JSON.stringify(Rimdian.currentDevice),OneYearInSeconds)}// 3. ID is created
  Rimdian._createDevice();},_createDevice:function _createDevice(){Rimdian.log("info","creating new device ID"),Rimdian.currentDevice={external_id:Rimdian.uuidv4(),created_at:new Date().toISOString(),user_agent:navigator.userAgent},Rimdian._setCookie("device",JSON.stringify(Rimdian.currentDevice),OneYearInSeconds);},// polyfill
  ...
  var a=Rimdian._getCookie("session");a&&Rimdian._setCookie("session",a,Rimdian.config.session_timeout);}},6e4),0<Rimdian.config.cross_domains.length)// loop over every <a> tag and add the decorateURL function
  ...
  ["gclid","fbclid","msclkid"].forEach(function(a){var b=Rimdian.getQueryParam(document.URL,a)||Rimdian.getHashParam(window.location.hash,a);b&&(f=b,g=a);}),"gclid"===g&&"referral"===b&&(b="ads"),a&&""!==a||(a="direct"),b&&""!==b||(b="none"),Rimdian.log("info","RMD utm_source is:",a),Rimdian.log("info","RMD utm_medium is:",b),Rimdian.log("info","RMD utm_campaign is:",c),Rimdian.log("info","RMD utm_content is:",d),Rimdian.log("info","RMD utm_term is:",e),Rimdian.log("info","RMD utm_id is:",f),Rimdian.log("info","RMD utm_id_from is:",g);// read session cookie
  var l=Rimdian._getCookie("session");// 1. no existing session -> create new session
  if(!l||""===l)return void Rimdian._startNewSession({utm_source:a,utm_medium:b,utm_campaign:c,utm_content:d,utm_term:e,utm_id:f,utm_id_from:g});// check if this origin should be ignored
  ...
  var n=JSON.parse(l);// check if the session was legacy
  if(Rimdian.log("info","RMD existing session is:",n),!0===Rimdian.config.from_cm&&void 0!==n.from){n={external_id:n.id,created_at:new Date().toISOString(),device_external_id:Rimdian.currentDevice.external_id,landing_page:window.location.href,referrer:Rimdian.getReferrer(),timezone:Rimdian.getTimezone(),utm_source:n.from.source||"direct",utm_medium:n.from.medium||"none",utm_campaign:n.from.campaign||void 0,utm_content:n.from.creative||void 0,utm_term:n.from.keyword||void 0,utm_id:n.from.gclid||void 0,utm_id_from:n.from.gclid&&""!==n.from.gclid?"gclid":void 0};// get session expiration date
  var o=Rimdian._getCookie("_cm_sessionExpiresAt");o&&""!==o&&(n.created_at=new Date(o).toISOString()),Rimdian._setCookie("session",JSON.stringify(Rimdian.currentSession),Rimdian.config.session_timeout);}// check if session origin has changed from previous page
  var p=!0;// 2. if this origin is ignored, or same origin, or empty origin, resume session
  return n.utm_source!==a&&(p=!1),n.utm_medium!==b&&(p=!1),n.utm_campaign!==c&&(p=!1),n.utm_content!==d&&(p=!1),n.utm_term!==e&&(p=!1),n.utm_id!==f&&(p=!1),m||p||!a||""===a?(Rimdian.log("info","RMD resume session (ignored:"+(m?"yes":"no")+", isEqual:"+p+", utm_source:"+a+")"),Rimdian.currentSession=n,void Rimdian._setCookie("session",JSON.stringify(Rimdian.currentSession),Rimdian.config.session_timeout)):void// 3. origin has changed, start new session
