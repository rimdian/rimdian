...
func (api *API) AppActivate(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/app.activate")
	...
	code, err := api.Svc.AppActivate(ctx, claims.AccountID, appDTO)
