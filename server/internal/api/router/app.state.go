...
func (api *API) AppMutateState(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/app.mutateState")
	...
	code, err := api.Svc.AppMutateState(ctx, claims.AccountID, appDTO)
