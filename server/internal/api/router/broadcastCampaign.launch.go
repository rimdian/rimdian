...
func (api *API) BroadcastCampaignLaunch(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/broadcastCampaign.launch")
	...
	code, err := api.Svc.BroadcastCampaignLaunch(ctx, claims.AccountID, data)
