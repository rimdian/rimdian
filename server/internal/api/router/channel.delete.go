...
func (api *API) ChannelDelete(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/channel.delete")
	...
	if err := json.Unmarshal(body, deleteChannelDTO); err != nil {
		api.ReturnJSONError(w, http.StatusBadRequest, err)
		return
	}

	updatedWorkspace, code, err := api.Svc.ChannelDelete(ctx, claims.AccountID, deleteChannelDTO)
