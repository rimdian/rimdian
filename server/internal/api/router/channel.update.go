...
func (api *API) ChannelUpdate(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/channel.update")
	...
	updatedWorkspace, code, err := api.Svc.ChannelUpdate(ctx, claims.AccountID, channelDTO)
