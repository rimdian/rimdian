...
func (api *API) DBAnalytics(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/db.analytics")
	...
	result, code, err := api.Svc.DBAnalytics(ctx, claims.AccountID, payload)
