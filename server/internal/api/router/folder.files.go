...
func (api *API) FolderFiles(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/folder.files")
	...
	result, code, err := api.Svc.FolderFiles(ctx, claims.AccountID, params)
