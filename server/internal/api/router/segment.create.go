...
func (api *API) SegmentCreate(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/segment.create")
	...
	code, err := api.Svc.SegmentCreate(ctx, claims.AccountID, segment)
