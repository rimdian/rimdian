...
func (api *API) SegmentDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(body, deleteSegmentDTO); err != nil {
		api.ReturnJSONError(w, http.StatusBadRequest, err)
		return
	}

	code, err := api.Svc.SegmentDelete(ctx, claims.AccountID, deleteSegmentDTO)
