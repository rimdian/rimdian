...
func (api *API) SegmentUpdate(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/segment.update")
	...
	if err := json.Unmarshal(body, segmentDTO); err != nil {
		api.ReturnJSONError(w, http.StatusBadRequest, err)
		return
	}

	code, err := api.Svc.SegmentUpdate(ctx, claims.AccountID, segmentDTO)
