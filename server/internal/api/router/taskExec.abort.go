...
func (api *API) TaskExecAbort(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/taskExec.abort")
	...
	taskDTO := &dto.TaskExecAbortParams{}
	...
	code, err := api.Svc.TaskExecAbort(ctx, claims.AccountID, taskDTO)
