...
func (api *API) WorkspaceCreate(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/workspace.create")
	...
	workspace, code, err := api.Svc.WorkspaceCreate(ctx, claims.AccountID, workspaceDTO)
