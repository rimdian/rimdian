...
func (api *API) WorkspaceUpdate(w http.ResponseWriter, r *http.Request) {

	ctx := r.Context()
	ochttp.SetRoute(ctx, "/api/workspace.update")
	...
	updatedWorkspace, code, err := api.Svc.WorkspaceUpdate(ctx, claims.AccountID, payload)
