...
func TaskExecRecomputeSegment(ctx context.Context, pipe *TaskExecPipeline) (result *entity.TaskExecResult) {
	...
	if segment.Version != segmentVersion {
		result.SetError(fmt.Sprintf("Segment %s, version %v has been updated (newer version building...)", segment.ID, segment.Version), true)
		return
	}
	...
	case entity.RecomputeSegmentStepExitUsers:
		...
		didActivate, err := pipe.Repo().ActivateSegment(bgCtx, pipe.Workspace.ID, segment.ID, segmentVersion)
